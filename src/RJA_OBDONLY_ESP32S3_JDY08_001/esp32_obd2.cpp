...
int OBD2Class::begin()
{
  Serial2.println("Starting CAN BUS");
  
  if (!CANMod.begin(CAN_BPS_500K)) {
    return 0;
  }
  ...
  CANMod.watchFor(0x7e8);

  if (!supportedPidsRead()) {
    // next try extended addressing
    _useExtendedAddressing = true;
    CANMod.watchFor(0x18daf110);

    if (!supportedPidsRead()) {

      Serial2.println("Supported PIDS read failed ...");
      
      return 0;
    }
  }

  Serial2.println("OBD BEGIN Success");
	...
	if ( CANMod.read(incoming) != 0 ) {
      ...
      if (incoming.data.uint8[0] == 0x10) {
        ...
        for (int pck = 0; read<length; pck++) {
          ...
          CANMod.sendFrame(outgoing);
		
          // wait for (proper) response
          while ( CANMod.read(incoming) != 0 || incoming.data.uint8[0] != (0x21 + pck)); // correct sequence number
