...
int ESP32CAN::_setFilter(uint32_t id, uint32_t mask, bool extended)
{
    ...
    if (debuggingMode) Serial2.println("Could not set filter!");
    return -1;
}

void ESP32CAN::_init()
{
    if (debuggingMode) Serial2.println("Built in CAN Init");
    for (int i = 0; i < BI_NUM_FILTERS; i++)
    {
        filters[i].id = 0;
        filters[i].mask = 0;
    ...
    {
        if (debuggingMode) Serial2.println("Initializing resources for built-in CAN\n");

                                 //Queue size, item size
        callbackQueue = xQueueCreate(16, sizeof(CAN_FRAME));
        rx_queue = xQueueCreate(rxBufferSize, sizeof(CAN_FRAME));
        if (debuggingMode) Serial2.println("Created queues.");

                  //func        desc    stack, params, priority, handle to task
        xTaskCreate(&task_CAN, "CAN_RX", 8192, this, 15, NULL);
        if (debuggingMode) Serial2.println("task rx created.");
        if (debuggingMode) Serial2.println("task low level rx created.");
        xTaskCreatePinnedToCore(&CAN_WatchDog_Builtin, "CAN_WD_BI", 2048, this, 10, NULL, 1);
        if (debuggingMode) Serial2.println("task watchdog created.");
        initializedResources = true;
    }
    if (debuggingMode) Serial2.println("_init done");
}
...
uint32_t ESP32CAN::init(uint32_t ul_baudrate)
{
    ...
    if (debuggingMode)
    {
        ...
        {
            Serial2.println("Alerts reconfigured\n");
        }
        else
        {
            Serial2.println("Failed to reconfigure alerts");
        }
    }
...
uint32_t ESP32CAN::beginAutoSpeed()
{
    ...
    while (valid_timings[idx].speed != 0)
    {
        ...
        disable();
        Serial2.print("Trying Speed ");
        Serial2.print(valid_timings[idx].speed);
        enable();
    ...
    Serial2.println("None of the tested CAN speeds worked!");
    twai_stop();
    return 0;
}
...
void ESP32CAN::enable()
{
    ...
    {
        Serial2.println("TWAI Driver started\n");
    }
    else
    {
        Serial2.println("Failed to start TWAI driver\n");
        return;
    }
    readyForTraffic = true;
}
...
bool ESP32CAN::processFrame(twai_message_t &frame)
{
    ...
    for (int i = 0; i < BI_NUM_FILTERS; i++)
    {
        ...
        if ((msg.id & filters[i].mask) == filters[i].id && (filters[i].extended == msg.extended))
        {
            ...
            if (debuggingMode) Serial2.write('_');
            return true;
        }
    }
    return false;
...
bool ESP32CAN::sendFrame(CAN_FRAME& txFrame)
{
    ...
    {
    case ESP_OK:
        if (debuggingMode) Serial2.write('<');
        break;
    case ESP_ERR_TIMEOUT:
        if (debuggingMode) Serial2.write('T');
        break;
